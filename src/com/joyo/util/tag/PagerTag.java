...
public class PagerTag extends SimpleTagSupport {
	...
	private void calcPage(StringBuilder str) {
		...
		if (this.totalPage <= 11){
			...
			//每页指定4条记录情形下
			//this.pageIndex + 8 >= this.totalPage
			else if (this.pageIndex + 20 >= this.totalPage){
			...
			/** 在中间 */
				...
				//每页4条
				//int i = this.pageIndex - 4; i <= this.pageIndex + 4; i++
				for (int i = this.pageIndex - 10; i <= this.pageIndex + 10; i++){
					if (this.pageIndex == i){
						/** 当前页码 */
						str.append("<span class='current'>"+ i +"</span>");
					}else{
